...
node. Temporary changes in the cluster state, such as placing nodes in and out of standby, will typically be performed
by logging into one node of the cluster (preferably one that is not degraded or about to undergo maintenance) and using
the pcs command line tool.

Any modifications to the cluster configuration, including resources, constraints, properties, and the BeeGFS services
themselves, should always be done using Ansible. It is crucial to maintain an up-to-date copy of the Ansible inventory
and playbook, ideally in source control to track changes, as part of the cluster maintenance process. When configuration
changes are needed, update the inventory and rerun the Ansible playbook that imports the BeeGFS HA role. 

The HA role will manage the process of placing the cluster into maintenance mode, making any necessary changes, and then
restarting BeeGFS or cluster services to apply the new configuration. As full node reboots are typically not needed
outside the initial deployment, this process is generally considered "safe". However, it's always recommended to perform
these operations during maintenance windows or off-hours in case any BeeGFS services need to restart. While these
restarts shouldn't typically cause application errors, but may impact performance, which some applications may handle
better than others.

In situations where you want to return the entire cluster back to a fully optimal state, or during emergencies where the
cluster is down, rerunning the Ansible playbook can be a quick and reliable recovery method, rather than pcs.
...
```
[root@host01 ~]# pcs status
Cluster name: hacluster
Cluster Summary:
  * Stack: corosync
  * Current DC: host01 (version 2.0.5-9.el8_4.3-ba59be7122) - partition with quorum
  * Last updated: Fri Jul  1 13:37:18 2022
  * Last change:  Fri Jul  1 13:23:34 2022 by root via cibadmin on host01
  * 6 nodes configured
...
Node List:
  * Node host06: standby
  * Online: [ host01 host02 host04 host05 ]
  * OFFLINE: [ host03 ]
```
...
Full List of Resources:
  * mgmt-monitor	(ocf::eseries:beegfs-monitor):	 Started host01
  * Resource Group: mgmt-group:
    * mgmt-FS1	(ocf::eseries:beegfs-target):	 Started host01
    * mgmt-IP1	(ocf::eseries:beegfs-ipaddr2):	 Started host01
    * mgmt-IP2	(ocf::eseries:beegfs-ipaddr2):	 Started host01
    * mgmt-service	(systemd:beegfs-mgmtd):	 Started host01
[...]
```
...
Fencing is enabled/required by default, see the [getting started](/docs/beegfs_ha/getting_started.md) guide for how to
generally configure fencing using Ansible.
...
For example in this sample inventory.yml host01 is the preferred file node to run the BeeGFS management service: 
...
Reversing the order would cause the management services to be preferred on host02: 
...
BeeGFS service you wish to relocate. For example to move the BeeGFS management service to host02 run: `pcs resource
move mgmt-monitor host02`. This process can be repeated to move one or more services away from their preferred
...
See [getting_started](/docs/beegfs_ha/getting_started.md) for details.
...
[...]
Jul 01 15:51:03 host01 pacemaker-schedulerd[9246]:  warning: Unexpected result (error: BeeGFS service is not active!) was recorded for monitor of meta_08-monitor on host02 at Jul  1 15:51:03 2022
```

In this instance BeeGFS service meta_08 stopped for some reason. To continue troubleshooting we should boot host02
...
[...]
Jul 01 16:18:01 host01 pacemaker-attrd[9245]:  notice: Node host02 state is now lost
Jul 01 16:18:01 host01 pacemaker-controld[9247]:  warning: Stonith/shutdown of node host02 was not expected
```
...
```
Jul 01 16:18:02 host01 pacemaker-schedulerd[9246]:  warning: Cluster node host02 will be fenced: peer is no longer part of the cluster
Jul 01 16:18:02 host01 pacemaker-schedulerd[9246]:  warning: Node host02 is unclean
Jul 01 16:18:02 host01 pacemaker-schedulerd[9246]:  warning: Scheduling Node host02 for STONITH
```
If the fencing action completes successfully you will see messages like: 
```
Jul 01 16:18:14 host01 pacemaker-fenced[9243]:  notice: Operation 'off' [2214070] (call 27 from pacemaker-controld.9247) for host 'host02' with device 'fence_redfish_2' returned: 0 (OK)
Jul 01 16:18:14 host01 pacemaker-fenced[9243]:  notice: Operation 'off' targeting host02 on host01 for pacemaker-controld.9247@host01.786df3a1: OK
Jul 01 16:18:14 host01 pacemaker-controld[9247]:  notice: Peer host02 was terminated (off) by host01 on behalf of pacemaker-controld.9247: OK
```
...
```
  * fence_redfish_2_monitor_60000 on host01 'not running' (7): call=2248, status='complete', exitreason='', last-rc-change='2022-07-26 08:12:59 -05:00', queued=0ms, exec=0ms
```
